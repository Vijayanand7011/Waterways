...
csv_file_pro = 'status_data.csv'
csv_file_soil_data =  'soil_data.csv'
...
def water_req_conv(mm):
    """
    Convert water depth in millimeters to liters per hectare.

    Args:
    mm (float): Water depth in millimeters.

    Returns:
    float: Water volume in liters per hectare.
    """
    return mm * 10000
...
def run_flask_app():
    ...
    def css_data_fetch_pro(csv_path_pro):
        ...
        w_mm = round(float(last_record['water_supplied']),2)
        w_l_ha = round(water_req_conv(float(last_record['water_supplied'])),2)
        # Prepare data to display in HTML
        data = {
            "title": "Predicted Output",
            "description_1": "On Consideration Of Field Parameters",
            "water_req": f"Water Required For Crop : {w_mm} mm (or) {w_l_ha} L/ha",
            "description_2":"Your Field Parameters :: ",
            "items": [
                f"Crop : {last_record['crop']}",
                f"Pincode : {last_record['pincode']}",
                f"Upper Temperature : {last_record['upper_temp']} °C",
                f"Lower Temperature : {last_record['lower_temp']} °C",
                f"Humidity : {last_record['humidity']} %",
                f"Rainfall : {last_record['rainfall']} mm",
                f"Wind Speed : {last_record['wind_speed']} m/s",
                f"Solar Radiation : {last_record['solar_rad']} MJ/m²",
                f"Soil Type : {last_record['soil_type']}",
                f"Soil Moisture : {last_record['soil_moisture']} %",
                f"Infiltration Rate : {last_record['infiltration_rate']} mm/hr",
                ...
                f"Root Depth : {last_record['root_depth']} cm",
                f"Crop Coefficient (Kc) : {last_record['crop_coeff']}",
                f"Irrigation Efficiency : {last_record['irrigation_efficiency']} %",
                f"Water Quality : {last_record['water_quality']}",  # You may want to specify a unit based on context (e.g., pH, EC)
